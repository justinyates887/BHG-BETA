module.exports = {
    ...
    execute(client, msg) {
        ...
        const track = client.player.nowPlaying(msg);
        const filters = [];

        Object.keys(client.player.getQueue(msg).filters).forEach((filterName) => client.player.getQueue(msg).filters[filterName]) ? filters.push(filterName) : false;
        ...
        msg.channel.send({
            embed: {
                ...
                fields: [
                    ...
                    { name: 'Volume', value: client.player.getQueue(msg).volume, inline: true },
                    { name: 'Repeat mode', value: client.player.getQueue(msg).repeatMode ? 'Yes' : 'No', inline: true },
                    { name: 'Currently paused', value: client.player.getQueue(msg).paused ? 'Yes' : 'No', inline: true },

                    { name: 'Progress bar', value: client.player.createProgressBar(msg, { timecodes: true }), inline: true }
                ],
